...
const statsHomeTitle = "AI Model Usage Rankings | 学霸帝Code Data"
const statsHomeDescription =
  "Explore 学霸帝Code Go usage across AI models, including token volume, rankings, market share, token pricing, session cost, cache ratio, and geo breakdowns."
const statsHomeFallbackUrl = "https://xuebadi-code.ai/data/"
...
function getStatsHomeUrl(base: string, requestUrl: string) {
  ...
  if (url.hostname === "stats.xuebadi-code.ai") return "https://xuebadi-code.ai/data/"
  if (url.hostname === "stats.dev.xuebadi-code.ai") return "https://dev.xuebadi-code.ai/data/"
  return url.toString()
}
  ...
  return (
    <section id="top-models" data-section="top-models">
      <h2 data-slot="top-models-title">
        <strong>Top models.</strong> <span>Usage of models across 学霸帝Code Go.</span>
      </h2>
      <Show
...
function UniqueUsersSection(props: { data: StatsHomeData["users"] }) {
  ...
  return (
    <section id="unique-users" data-section="unique-users">
      ...
      <SectionTitle title="Unique Users" description="Daily unique 学霸帝Code Go users by model." />
      <Show
        when={data().some((item) => usageTotal(item) > 0)}
        fallback={
